...
Бот собирает новости Fortnite из множества источников, переписывает их на русском языке в официальном новостном стиле с помощью LLM, генерирует баннеры через Replicate и публикует посты с кнопкой пополнения V-Bucks.
...
- **Написание постов** — переписывает новости на русском через LLM (gpt-5.5 / wellflow.dev)
- **Генерация изображений** — создаёт 16:9 баннеры через Replicate (openai/gpt-image-2)
- **Публикация** — отправляет фото + текст в канал с inline-кнопкой «Пополнить V-Bucks»
...
|---|---|---|
| `LLM_API_URL` | Базовый URL API (OpenAI-совместимый) | `https://api.wellflow.dev` |
| `LLM_API_KEY` | API-ключ для аутентификации | — |
| `LLM_MODEL` | Модель для генерации текста | `gpt-5.5` |

### Генерация изображений (Replicate)

| Переменная | Описание |
|---|---|
| `REPLICATE_API_TOKEN` | API-токен с [replicate.com](https://replicate.com) |
...
6. **Написание** — LLM (gpt-5.5 через wellflow.dev) переписывает новость на русском по одному из 5 шаблонов
7. **Генерация изображения** — Replicate (`openai/gpt-image-2`) создаёт 16:9 баннер
8. **Публикация** — отправка фото + текста + кнопки «Пополнить V-Bucks» в канал @FortnitebucksShop
